...
describe("when there is initially one user at db", () => {
  ...
  test("get users returns it", async () => {
    const usersAtStart = await helper.usersInDb();
    const usersReturned = await api.get("/api/users").expect(200);
  ...
  test("with too short username fails with correct error message", async () => {
    ...
    assert(
      result.body.error.includes(
        "User validation failed: username: Path `username` (`aa`, length 2) is shorter than the minimum allowed length (3).",
      ),
    );
    assert.deepStrictEqual(usersAtEnd, usersAtStart);
  });
